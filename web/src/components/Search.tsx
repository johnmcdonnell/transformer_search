...
function render_results(results: Array<SearchProps['results'][0]>) {
    if (results.length > 0) {
        return (
...
class Search extends React.Component<{}, SearchProps> {
    ...
    render () {
        ...
        const renderable_results = render_results(results);
        const is_loading = this.state.loading;

        return (
            <div>
                <Typography variant="h1">
                    Search Marginal Revolution
                </Typography>
                <form 
                    onSubmit={(e) => {
                        e.preventDefault();
