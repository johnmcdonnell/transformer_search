...
const backend_url = 'https://vibecheck-backend-marginalrevolution.onrender.com'
...
class Hits extends React.Component<{hits: SearchProps['hits']}> {
    render() {
            ...
            return (
                ...
                >
                    ...
                    <List sx={{backgroundColor: 'secondary.main', 'borderRadius': '10px'}}>
                        {this.props.hits.map((hit) => <ListItem key={hit.result_number}><SearchResult result={hit} /></ListItem>)}
                    </List>
                </Container>
            )
        } else {
...
function SearchResult(props: {'result': SearchProps['hits'][0]}) {
    return (
...
class Search extends React.Component<{}, SearchProps> {
        ...
        fetch(`${backend_url}/summarize_results`, {
            ...
            headers: {
                ...
                body: JSON.stringify({
                    ...
                    hit_uris: this.state['hits'].map((result) => result.link)
                    })
            })
    ...
    render () {
        const {hits, summary, loading_hits, loading_summary} = this.state;
        return (
            <div>
                <form 
                    onSubmit={(e) => {
